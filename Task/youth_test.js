...
let cookieArr = [], cookie = '',
    ...
    Object.keys(ARTBODYs).forEach((item) =>{
	    ...
	    Object.keys(SHAREBODYs).forEach((item) =>{
        if (SHAREBODYs[item]) {
            shareArr.push(SHAREBODYs[item])
        }
    });
...
function ArticleShare() {
    return new Promise((resolve, reject) => {
        $.post(batHost('article/share/put.json',sharebodyVal), (error, response, data) => {
        try{
           sharego = JSON.parse(data);
     if (data.indexOf('score')>-1&&sharego.items.score!=0)  {
              detail += `【阅读分享】+${sharego.items.score}个青豆\n`;
             $.log(`阅读分享 +${sharego.items.score}个青豆\n`)
            } 
         } catch(e) {
          $.logErr(e+resp);
        } finally {
