...
if(!$.isNode()&&ysmhd.indexOf("\n") ==-1){
    ...
    if (process.env.YSM_HD && process.env.YSM_HD.indexOf('\n') > -1) {
        ysmhd = process.env.YSM_HD.split('\n');
    } else {
        ysmhd = [process.env.YSM_HD]
    };
    if (process.env.YSM_URL && process.env.YSM_URL.indexOf('\n') > -1) {
        ysmurl = process.env.YSM_URL.split('\n');
    } else {
        ysmurl = [process.env.YSM_URL]
    };
    if (process.env.YSM_BODY && process.env.YSM_BODY.indexOf('\n') > -1) {
        ysmbody = process.env.YSM_BODY.split('\n');
    } else {
        ysmbody = [process.env.YSM_BODY]
    };    
    if (process.env.YSM2_BODY && process.env.YSM2_BODY.indexOf('\n') > -1) {
        ysm2body = process.env.YSM2_BODY.split('\n');
    } else {
        ysm2body = [process.env.YSM2_BODY]
    };	
    if (process.env.YSM_TX && process.env.YSM_TX.indexOf('\n') > -1) {
        ysmtx = process.env.YSM_TX.split('\n');
    } else {
        ysmtx = [process.env.YSM_TX]
    };	
	console.log(` ============脚本执行 - 北京时间 (UTC + 8)：${new Date(new Date().getTime() + 8 * 60 * 60 * 1000).toLocaleString()} =============\n`);
 } else if(!$.isNode()&&ysmhd.indexOf("\n")>-1){
   ysmhd = ysmhd.split("\n")
   ysmurl = ysmurl.split("\n")
   ysmbody = ysmbody.split("\n")
   ysm2body = ysm2body.split("\n")
   ysmtx = ysmtx.split("\n")
};
